...
void displayRecordsInHash(struct HashType *hashTable, int tableSize)
{
    ...
    /*checking current which starts at head of hashtable
    traversing while current is not null
        while loop going through list, current next is not null
            printing data based on record type
            current becomes next to follow through
        print last item
    */
    struct RecordType *temp;
    ...
    for (int i=0; i < tableSize; i++){
        ...
        printf("\nIndex %d", i);
        while(temp != NULL){
            printf("\n %d, %c, %d", temp->id, temp->name, temp->order);
            temp = temp->next;
        }
    }
}
...
int main(void)
{
    ...
    struct HashType *newTable = calloc(hashTableSize, sizeof(struct HashType));

    // for each record in pRecords, insert it into the hash table using the insertRecord function
    for (int i=0; i<recordSz; i++){
        insertRecord(newTable, &pRecords[i], hashTableSize);
    }
    // call the display records function
    displayRecordsInHash(newTable, hashTableSize);
